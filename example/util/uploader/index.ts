...
export type UploadItem = { idx: string, url: string, status: 'pending' | 'done' | 'error'; };
...
export class Uploader {
	...
	public async uploadFiles(files: File[]) {
		...
		const uploadList: UploadItem[] = results.map(item => ({ url: '', status: 'pending', idx: `uploader-${uniqueId()}` }));
