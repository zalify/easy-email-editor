  ...
  return generateAdvancedBlock(__spreadProps(__spreadValues({}, option), {
    ...
    getContent: (params) => {
      ...
      if (mode === "testing" || !(i18n == null ? void 0 : i18n.enabled)) {
        ...
        for (const match of matches) {
          if (i18n.type === I18nType.NI18N || i18n.type === I18nType.CNI18N) {
            if (match[1] === i18n.numberVariable) {
              ...
              content = content.replace(match[0], "{%" + (counter + 1) + "=" + match[1] + "}");
              counter++;
        ...
        if (i18n.type === I18nType.NI18N || i18n.type === I18nType.CNI18N) {
          const matches2 = singularText.matchAll(regexPattern);
          let counter2 = 1;
          for (const match of matches2) {
            if (match[1] === i18n.numberVariable) {
              singularText = singularText.replace(match[0], "1");
            } else {
              singularText = singularText.replace(match[0], "{%" + counter2 + "=" + match[1] + "}");
            }
          }
        }
