...
const MonacoEditor = React.lazy(() => import('@/components/EmailEditor/components/MonacoEditor'));

export const HtmlEditor: React.FC<{ visible: boolean, setVisible: (v: boolean) => void; }> = (props) => {
      ...
      title={
        (
          <Stack distribution="equalSpacing">
            <TextStyle variation="strong" size="large">Html</TextStyle>
            <Stack>
    ...
    >
      <div style={{ display: 'flex', height: '100%' }}>
        <div style={{ flex: 1, height: '100%' }}>
          ...
          <Suspense fallback={<div style={{ height: '100%', width: '100%', display: 'flex', alignItems: 'center', backgroundColor: 'rgb(40, 44, 52)', justifyContent: 'center', fontSize: 24, color: '#fff' }}>Editor Loading...</div>}>
            <MonacoEditor height="100%" value={content} onChange={setContent} />

          </Suspense>
        </div>
        <div style={{ flex: 1, height: '100%', overflow: 'auto', marginRight: 10 }}>
          <ShadowDom style={{ ...styles, width: pageData.attributes.width || '600px', margin: 'auto' }}>
